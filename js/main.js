...
//count
let count = 0; // Número inicial
const maxCount = 10; // Número máximo al que llegará el contador
const countupElement = document.getElementById("countup"); // Elemento donde se mostrará el número
...
function startCountup() {
  const interval = setInterval(() => {
    if (count < maxCount) {
      count++;
      countupElement.textContent = count; // Actualiza el número en el HTML
    } else {
      clearInterval(interval); // Detiene el contador cuando llega a 10
    }
  }, 1000); // Aumenta cada 1 segundo (1000 ms)
}
...
function parallax(e) {
  ...
  let x = e.clientX / 70 - 30;
  let y = e.clientY / 70 - 30;
...
function unlock() {
  ...
  document.querySelector(".e1-1").style.visibility = "hidden";
  document.querySelector(".e1-2").style.visibility = "hidden";
  document.querySelector(".e1-3").style.visibility = "hidden";
  apps();
}

function apps() {
  let tar = document.querySelector(".sc");
  tar.innerHTML += '<div class="space"></div>';
  for (i = 0; i < 24; i++) {
    tar.innerHTML += '<div class="app"></div>';
  }
  fadeIn();
}
